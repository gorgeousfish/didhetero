    ...
    real colvector t_vals)
{
    ...
    if (pretrend != 0) {
        // Base-period exclusion t = g - delta - 1 (eq. 2.32-2.33).
        if (t_ord == g_ord - anticipation - 1) return(0)

        // Pre-treatment pairs (t < g - delta): comparison pool anchored at
        // t = g (R_{g,g}, Appendix F).  Under notyettreated without a
        // never-treated pool, require some cohort treated strictly after
        // g + delta to serve as controls.
        if (t_ord < g_ord - anticipation &
        ...
        // Remaining pairs (post-treatment pairs, plus pre-treatment pairs that
        // rely on a never-treated pool or nevertreated controls) must satisfy
        // the same comparison-group support condition as the main branch,
        // because Assumption 2.5' imposes s < gbar on both regimes.
